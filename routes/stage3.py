...
@stage3_bp.route('/stage3/products', methods=['GET'])
def products():
    sort = request.args.get('sort', 'name')
    # Vulnerable to ORDER BY injection
...
@stage3_bp.route('/stage3/submit', methods=['POST'])
def submit_flag():
    data = request.get_json()
    if data and data.get('flag') == 'flag{sqli_blind_injection}':
        mark_solved(3)
        return jsonify({'solved': True, 'flag': data.get('flag'), 'next_stage': 4})
